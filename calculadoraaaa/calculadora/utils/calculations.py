...
Aqui separamos a lógica de cálculo das interfaces gráficas.
"""

def calcular_consumo_energia(potencia_w: float, tempo_h: float, dias: int) -> float:
    """
    Calcula o consumo de energia em kWh por mês.
    Fórmula: (Potência em Watts * Horas de Uso por Dia * Dias de Uso) / 1000
    """
    try:
...
def calcular_media_tres_valores(v1: float, v2: float, v3: float) -> float:
    """
    Calcula a média aritmética simples de 3 valores.
    Fórmula: (v1 + v2 + v3) / 3
    """
    try:
        media = (v1 + v2 + v3) / 3.0
...
def calcular_imc_status(peso_kg: float, altura_m: float) -> tuple[float, str]:
    """
    Calcula o Índice de Massa Corporal (IMC) e fornece a classificação oficial da OMS.
    Fórmula: peso / (altura * altura)
    Retorna uma tupla (valor_imc, status_formatado).
    """
    try:
